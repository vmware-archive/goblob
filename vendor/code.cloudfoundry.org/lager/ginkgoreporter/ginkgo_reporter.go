...
func New(writer io.Writer) *GinkgoReporter {
	logger := boshlog.NewLogger("ginkgo")
	logger.RegisterSink(boshlog.NewWriterSink(writer, boshlog.DEBUG))
...
func (g *GinkgoReporter) SpecWillRun(specSummary *types.SpecSummary) {
	g.wrappedWithNewlines(func() {
		...
		g.session.Info("start", boshlog.Data{
			"summary": SpecSummary{
...
func (g *GinkgoReporter) SpecDidComplete(specSummary *types.SpecSummary) {
	g.wrappedWithNewlines(func() {
		...
		if passed(specSummary.State) {
			g.session.Info("end", boshlog.Data{
				"summary": summary,
			})
		} else {
			summary.StackTrace = specSummary.Failure.Location.FullStackTrace
			g.session.Error("end", errorForFailure(specSummary.Failure), boshlog.Data{
				"summary": summary,
			})
		}
		g.session = nil
